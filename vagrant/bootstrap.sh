...
SITE_DIR="/var/www/html/sass.local" # The path to the web application on the server (default: /var/www/html)
SITE_PUB_DIR="/var/www/html/sass.local/public" # The path to the application public web folder.
VAGRANT_DATA_DIR="/vagrant/vagrant"
DB_NAME="sass" #name of postgres DB.
...
#su postgres
#psql -f $VAGRANT_DATA_DIR/postgresql/sass-db-init.backup
#exit
runuser -l postgres -c 'psql -f /vagrant/vagrant/postgresql/sass-db-init.backup'

#Add sass user to linux system so laravel can connect via ident instead of trust
...
#ln -s $VAGRANT_DATA_DIR/nginx/sass.local /etc/nginx/sites-enabled/sass.local

mv /etc/nginx /etc/nginx-orig
ln -s $VAGRANT_DATA_DIR/nginx/server-configs-nginx-master /etc/nginx
ln -s /etc/nginx/sites-available/sass.local /etc/nginx/sites-enabled/sass.local
...
php /var/www/html/sass.local/artisan migrate
